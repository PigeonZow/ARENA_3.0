...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        residual = (residual - residual.mean(dim=-1, keepdim=True)) / (residual.var(dim=-1, keepdim=True, unbiased=False) + self.cfg.layer_norm_eps).sqrt()
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        # Calculate query, key and value vectors. To get this, we multiply the input by the weight matrices W_Q, W_K, W_V, and add the biases b_Q, b_K, b_V. We can do this efficiently by using einsum.

        # query: represent what the current position in the sequence is looking for in a key k (ex. token of "cat" in "the cat sat on the mat"). q for "cat" is compared to k of other tokens like "sat" to find matching keys and generate attention probabilities. 
        q = (
            einops.einsum(
        ...
        # key: represents what the current position in the sequence has to offer to other positions in the sequence (ex. what "cat" in "the cat sat on the mat" means for other tokens like "sat"). the dot product of q for "cat" and k for "sat" gives the attention score for "cat" looking at "sat".
        k = (
            einops.einsum(
        ...
        # value: represent some information about the current position in the sequence. once the relevant keys are found by comparing q and k, the value vector is used to generate the output. for "cat", this would be the actual information "cat" provides. 
        v = (
            einops.einsum(
    ...
    def apply_causal_mask(
        ...
        """
        ones = t.ones(attn_scores.size(-2), attn_scores.size(-1), device=attn_scores.device)
        mask = t.triu(ones, diagonal=1).bool()
        masked_scores = t.where(mask == True, self.IGNORE, attn_scores)
        # print(f"scores: {masked_scores}")
